...
def main():
    ...
    # --- initialize dataset --- #
    X_train, y_train = load_data_resampling(mode,'train',len_seq,stride,remove=remove_null)
    X_val, y_val = load_data_resampling(mode,'val',len_seq,stride,remove=remove_null)
    ...
    # --- initialize model --- #
    net = DeepConvLSTM(n_channels=n_channels, n_classes=18)
    ...
    with open('log.csv', 'w', newline='') as csvfile:
        for e in range(num_epochs):
            ...
            early_stopping(np.mean(val_losses), net, exp_name)
    ...
    X_test, y_test = load_data_resampling(mode,'test',len_seq,stride=1,remove=remove_null)
    x_test_NULL, y_test_NULL = get_null_data(mode,'test', len_seq, stride=1, label= 0)
    X_test = X_test + x_test_NULL
    y_test = y_test + y_test_NULL
